...
slope_eff=emtrends(mod_eff, ~month,var="ln_cfs")%>%
...
plot_dat=slope_eff%>%
  mutate(across(c(estimate,asymp.LCL,asymp.UCL)))%>%
  mutate(across(where(is.numeric), ~round(.,4)))%>%
  mutate(metric=factor(metric,levels=c("Monthly effect of discharge on effort")),
...
#ggsave(plot=month_eff_plot,"effort.png",height=4,width=7,bg="white")
...
## Export
write_csv(eff_app,"effort model summary.csv")