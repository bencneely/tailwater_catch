...
counts=read_csv("counts_clean.csv")
fish=read_csv("fish_genus_clean.csv")
discharge=read_csv("discharge_clean.csv")
...
## Look at number of counts for effort model and interviews for catch model
sum(counts1$totangs)
length(unique(fish1$interview_id))

## Angler count summary for effort
tmp3=counts1%>%
  group_by(impd,year)%>%
...
## Angler interview summary for catch
tmp4=fish1%>%
  group_by(impd,year)%>%
  summarize(n=n_distinct(interview_id),
            .groups="drop")
...
## Interviews and total catch rate per month
## First get a total catch rate for each party
fish2=fish1%>%
...
## Summarize mean number of interviews and sd per month
out2=fish2%>%
  group_by(impd,month,year)%>%
  summarize(ints=n_distinct(interview_id),
            .groups="drop")%>%
  group_by(month)%>%
  summarize(mean_ints=mean(ints),
            sd_ints=sd(ints))
print(out2)

## Summarize mean catch rate and sd per month 
out3=fish2%>%
  group_by(month)%>%
  summarize(mean_cr=mean(catch_rate,na.rm=T),
